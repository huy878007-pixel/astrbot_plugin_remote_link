...
一开始只是想让群里的机器人能画张图，是山根乡的群友们一句句「能不能也生成视频」「能不能用我自己的工作流」「这个提示词能不能自动优化」，一点点把它推到了今天的样子。你们踩的每一个坑、报的每一个 bug、提的每一个「要是能……就好了」，都写进了这份代码和这份文档里。

没有你们的折腾、催更和不留情面的吐槽，本喵大概还窝在窗台上晒太阳，这个插件也不会存在。

**谢谢你们，山根乡的各位。** 这个插件属于你们每一个人 (´▽｀)

### 也感谢

- **[AstrBot](https://github.com/AstrBotDevs/AstrBot)** —— 一个足够开放的插件平台，让这种「云端 ↔ 本地」的疯狂想法有地方落地
